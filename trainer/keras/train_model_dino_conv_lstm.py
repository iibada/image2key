...
def sliding_window_generator(X, Y, window_size, batch_size=32, epochs=10):
    for epoch in range(epochs):
        for i in range(len(X) - window_size):
            ...
            for j in range(batch_size):
                prev_keys.append(Y[i + j:i + j + window_size])
                current_frame.append(X[i + j + window_size])
                current_key.append(Y[i + j + window_size])
                
            yield [np.asarray(prev_keys), np.asarray(current_frame)], np.asarray(current_key)
...
if __name__ == "__main__":
    ...
    model = model_arch.get_conv_lstm_model(frame_size, width, height, all_key_weights_dino)
    print(model.summary())
    ...
    data_gen = sliding_window_generator(X, Y, frame_size, batch_size=batch_size, epochs=epochs)
