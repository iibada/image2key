...
if __name__ == "__main__":
    ...
    model_dir = "./models/pytorch/dino/lstm_efficientNetV2"
    model = torch_models.LSTM_EfficientNetV2(data_settings=data_settings).to(device)
    model_state_dict = torch.load(f"{model_dir}/model.pt").state_dict()
    ...
    while True:
        ...
        prediction = model(prev_keys, current_frame)

        result = torch.argmax(prediction).item()

        prev_keys = torch.roll(prev_keys, -1)
        prev_keys[0][-1] = torch.tensor(
            [0] * len(base_key.all_key_and_type_comb), dtype=torch.float32
        )
        prev_keys[0][-1][result] = torch.tensor(1, dtype=torch.float32)
