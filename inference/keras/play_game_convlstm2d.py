...
if __name__ == '__main__':
    ...
    model_dir = "./models/keras/dino/convlstm2d"
    ...
    prev_frames = np.asarray([img] * FPS)
    prev_keys = np.asarray([[1] + ([0] * (len(base_key.all_key_and_type_comb) - 1))] * FPS)
    ...
    while True:
        ...
        a_img = np.asarray(image.resize((width, height)))
        current_frame = np.asarray(a_img / 255, dtype=np.float16)
        
        X = [np.array([prev_frames]), np.array([prev_keys]), np.array([current_frame])]
        prediction = model.predict(X)
        
        result = np.argmax(prediction)

        prev_frames = np.roll(prev_frames, -1)
        prev_frames[-1] = current_frame
        prev_keys = np.roll(prev_keys, -1)
        prev_keys[-1] = [0] * len(base_key.all_key_and_type_comb)
        prev_keys[-1][result] = 1
