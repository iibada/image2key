...
def scatter(x, colors, file_name, class_num):
    ...
    for i in range(class_num):
        ax.scatter(
            x[colors == label_set[i], 0],
            x[colors == label_set[i], 1],
            lw=0,
            s=70,
            c=color_pen[i],
        ...
        for i in range(layers):
            ...
            for j in range(c[layers * t + i].shape[0]):
                for k in range(c[layers * t + i].shape[1]):
                    ...
                    c_key = (
                        F.normalize(
                            torch.cat([value1, c[layers * t + i, j, k, index2]], dim=0),
                            dim=0,
                        )
                        .detach()
                        .cpu()
                    ...
                    m_key = (
                        F.normalize(
                            torch.cat([m[layers * t + i, j, k, index1], value2], dim=0),
                            dim=0,
                        )
                        .detach()
                        .cpu()
                ...
                plot_TSNE(
                    ...
                    np.array(label),
                    path,
                    "case_" + str(j) + "_tsne_" + str(i) + "_" + str(t),
                    2,
                )
